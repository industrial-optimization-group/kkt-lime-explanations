...
def river_pollution_problem(five_obj: bool = False):
    ...
    objective_1 = ScalarObjective(name="DO in the city", evaluator=f_1)
    objective_2 = ScalarObjective(name="DO at the downstream state line", evaluator=f_2)
    objective_3 = ScalarObjective(name="Fishery's return on investment", evaluator=f_3)
    objective_4 = ScalarObjective(name="City's additional tax rate", evaluator=f_4)
...
def discrete_river_pollution():

    filename = (
        os.path.abspath(os.getcwd()) + "/explainable_moo/data/river_pollution_10178.csv"
    )
    ...
    problem = DiscreteDataProblem(df, ["x_1", "x_2"], objective_names, ideal, nadir)
